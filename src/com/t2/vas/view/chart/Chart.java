...
public class Chart extends View implements OnLongClickListener, OnClickListener {
	...
	private void initChartDrawables() {
		Series ls = this.getLargestSeries();

		// Calculate the point width
		int conWidth = this.chartContainer.width();
		int conHeight = this.chartContainer.height();
		int maxDim = (conWidth < conHeight)?conHeight:conWidth;
		int totalPoints = ls.getValues().size();
		float pointPaddingRightRatio = 1.0f;

		this.pointWidth = (int) (maxDim / (totalPoints * (pointPaddingRightRatio + 1)));
		this.pointPaddingRight = (int) (pointWidth * pointPaddingRightRatio);

		if(this.pointWidth > (maxDim / 20)) {
			this.pointWidth = (maxDim / 20);
			this.pointPaddingRight = (int) (pointWidth * pointPaddingRightRatio);
		}
